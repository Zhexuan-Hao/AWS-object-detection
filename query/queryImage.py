...
def lambda_handler(event, context):
    upload_image(event["imageData"]["image"])
    image_tags = do_object()

    tags_counter = Counter(image_tags)
    print("tags_counter: ", tags_counter)
    response = dynamodb.scan()
    items = response.get("Items", [])
    
    rlt = []
    for item in items:
        item_tags = (i["label"] for i in item["Tags"]["objects"])
        item_tag_counter = Counter(item_tags)
        print("item_tag_counter:", item_tag_counter)
        is_valid = True
        for k, v in item_tag_counter.items():
            if tags_counter[k] > v:
                is_valid = False
                break
        
        if is_valid:
            item_rlt_tags = [{"tag": v, "count": v} for k, v in item_tag_counter.items()]
            rlt.append({
                "url": item["URL"],
                "filename": item["Key"],
                "tags":item_rlt_tags
